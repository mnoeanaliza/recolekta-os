...
export default function App() {
  ...
  // Formulario actualizado con mLlegada y mSalida
  const [form, setForm] = useState({ recolector: '', sucursal: '', area: '', tipo: '', hLlegada: '08', mLlegada: '00', pLlegada: 'AM', hSalida: '08', mSalida: '00', pSalida: 'AM', observaciones: '' });
  const [suggestions, setSuggestions] = useState({ transportistas: [], sucursales: [] });
  const [activeInput, setActiveInput] = useState(null);

  useEffect(() => {
    signInAnonymously(auth).catch(() => console.warn("Modo offline o error de auth"));
    if (!localStorage.getItem('recolekta_tutorial_v25')) setShowWelcome(true);
  ...
  const metrics = useMemo(() => {
    ...
    try {
      ...
      const monthlyData = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'].map((m, i) => {
        ...
        const ef = mRecs.length > 0 ? (mRecs.filter(d => (d.tiempo || 0) <= 5).length / mRecs.length) * 100 : 0;
        return { name: m, ef: parseFloat(ef.toFixed(1)), count: mDocs.length };
      }).filter(d => d.count > 0);
  ...
  const downloadReport = () => {
    ...
    const reporteTitle = filterUser === 'all' ? "REPORTE CONSOLIDADO" : "FICHA INDIVIDUAL: " + filterUser;
    doc.text(reporteTitle, 20, 60);
    ...
    doc.save(`Recolekta_${filterUser}.pdf`);
  };
  ...
  const getWait = () => { 
    const s = timeToMins(form.hLlegada, form.mLlegada, form.pLlegada);
    const e = timeToMins(form.hSalida, form.mSalida, form.pSalida); 
    return (e-s)>=0?e-s:0; 
  };
    ...
    <div className="min-h-screen bg-[#F4F7FE] text-slate-800 font-sans pb-10">
      ...
      {showWelcome && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white max-w-md w-full rounded-[2.5rem] p-8 text-center shadow-2xl relative animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tighter mb-4">¡Hoja de Ruta Digital!</h2>
            <p className="text-slate-500 text-sm mb-10 font-medium leading-relaxed">Registra tus tiempos de forma fácil y sube tu evidencia fotográfica en cada punto.</p>
            <button onClick={() => { setShowWelcome(false); localStorage.setItem('recolekta_tutorial_v25', 'true'); }} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg flex items-center justify-center gap-3">Empezar Ahora <ChevronRight size={16}/></button>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white p-6 md:p-10 rounded-2xl md:rounded-[3rem] shadow-xl border-t-[10px] border-green-500">
              ...
              <form onSubmit={async (e) => { e.preventDefault(); try { const isR = RECOLECCION_STRINGS.some(s=>form.tipo.toLowerCase().includes(s)); await addDoc(collection(db, "registros_produccion"), { ...form, tiempo: getWait(), createdAt: new Date().toISOString(), categoria: isR ? "Recolección" : "Diligencia" }); alert("¡Sincronizado!"); setForm({...form, sucursal: ''}); } catch(e) {alert("Error de Envío");} }} className="space-y-6">
                ...
                {/* --- SECCIÓN: RELOJ CON MINUTOS --- */}
                <div className="bg-slate-900 p-6 md:p-8 rounded-2xl md:rounded-3xl text-white flex flex-col sm:flex-row justify-around items-center gap-6">
                  <div className="flex gap-4 md:gap-8">
                    <div>
                      <p className="text-[9px] font-bold text-green-400 mb-2 uppercase tracking-tighter">Entrada</p>
                      <div className="flex gap-1">
                        <select className="bg-slate-800 p-2 rounded-lg font-black text-sm" value={form.hLlegada} onChange={e=>setForm({...form, hLlegada:e.target.value})}>{Array.from({length: 12},(_,i)=>String(i+1).padStart(2,'0')).map(h=><option key={h}>{h}</option>)}</select>
                        <select className="bg-slate-800 p-2 rounded-lg font-black text-sm" value={form.mLlegada} onChange={e=>setForm({...form, mLlegada:e.target.value})}>{Array.from({length: 60},(_,i)=>String(i).padStart(2,'0')).map(m=><option key={m}>{m}</option>)}</select>
                        <select className="bg-green-600 p-2 rounded-lg text-[10px]" value={form.pLlegada} onChange={e=>setForm({...form, pLlegada:e.target.value})}><option>AM</option><option>PM</option></select>
                      </div>
                    </div>
                    <div>
                      <p className="text-[9px] font-bold text-orange-400 mb-2 uppercase tracking-tighter">Salida</p>
                      <div className="flex gap-1">
                        <select className="bg-slate-800 p-2 rounded-lg font-black text-sm" value={form.hSalida} onChange={e=>setForm({...form, hSalida:e.target.value})}>{Array.from({length: 12},(_,i)=>String(i+1).padStart(2,'0')).map(h=><option key={h}>{h}</option>)}</select>
                        <select className="bg-slate-800 p-2 rounded-lg font-black text-sm" value={form.mSalida} onChange={e=>setForm({...form, mSalida:e.target.value})}>{Array.from({length: 60},(_,i)=>String(i).padStart(2,'0')).map(m=><option key={m}>{m}</option>)}</select>
                        <select className="bg-orange-600 p-2 rounded-lg text-[10px]" value={form.pSalida} onChange={e=>setForm({...form, pSalida:e.target.value})}><option>AM</option><option>PM</option></select>
                      </div>
                    </div>
                  </div>
                  <div className="text-center sm:text-right border-t sm:border-t-0 sm:border-l border-slate-700 w-full sm:w-auto pt-4 sm:pt-0 sm:pl-8 font-black"><p className="text-[10px] text-slate-500 uppercase leading-none mb-1">Espera Total</p><h4 className={cn("text-3xl md:text-5xl tracking-tighter transition-colors", getWait() > 5 ? "text-orange-400" : "text-green-400")}>{getWait()}m</h4></div>
                </div>
                {/* --- FIN RELOJ --- */}

                <label className="w-full p-8 bg-slate-50 border-2 border-dashed rounded-xl md:rounded-3xl flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-white transition-all text-slate-400 font-bold uppercase text-[9px] shadow-sm"><Camera size={24}/><p>Capturar Foto Obligatoria</p><input type="file" className="hidden" accept="image/*" required /></label>
                <button type="submit" className="w-full bg-slate-900 text-white py-5 md:py-7 rounded-2xl md:rounded-3xl font-black text-xl shadow-xl hover:scale-[1.01] transition-all flex items-center justify-center gap-4 uppercase tracking-tighter">Sincronizar a Nube <CheckCircle2 size={24} className="text-green-500"/></button>
              </form>
            </div>
            <div className="space-y-6">
              <div className="bg-slate-900 p-8 md:p-10 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden"><p className="text-[10px] font-black text-slate-500 uppercase mb-4 tracking-widest leading-none">Tu Eficiencia Hoy</p><h4 className="text-5xl md:text-7xl font-black text-green-500 mb-2 leading-none">{(liveData.filter(d => d.recolector === form.recolector && (d.tiempo||0) <= 5).length / (liveData.filter(d => d.recolector === form.recolector && d.categoria === 'Recolección').length || 1) * 100).toFixed(0)}%</h4><TrendingUp className="absolute -right-10 -bottom-10 text-white opacity-5" size={200}/></div>
              <button onClick={() => { if(!form.recolector) return alert("Escribe tu nombre arriba"); setFilterUser(form.recolector); downloadReport(); }} className="w-full bg-white p-5 rounded-2xl border-2 text-slate-600 font-black text-[11px] uppercase flex items-center justify-center gap-3 shadow-md hover:bg-slate-50 active:scale-95 transition-all"><Download size={18}/> Mi Ficha Individual</button>
            </div>
          </div>
        ) : (
          /* MODO ADMIN */
             ...
             {metrics?.total > 0 ? (
               <div className="space-y-6 md:space-y-12">
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                   <KPICard title="Eficiencia" value={`${metrics.eficiencia}%`} subtitle="Meta 95% (≤5m)" icon={TrendingUp} color="bg-green-600" />
                   <KPICard title="Promedio" value={`${metrics.promRec}m`} subtitle="Muestras Médicas" icon={Clock} color="bg-blue-600" />
                   <KPICard title="Diligencias" value={metrics.dilsCount} subtitle="Volumen Periodo" icon={BarChart3} color="bg-orange-500" />
                   <KPICard title="Registros" value={metrics.total} subtitle={dataSource==='live'?'Firebase':'GitHub Historial'} icon={Database} color="bg-slate-800" />
                </div>
