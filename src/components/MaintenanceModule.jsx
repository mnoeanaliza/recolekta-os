...
export default function MaintenanceModule({ currentUser, sysConfig }) {
    ...
    useEffect(() => {
        ...
        const q = hasFleetPeriod
            ? query(collection(db, "registros_mantenimiento"), where("usuario", "==", currentUser.email), where("fecha", ">=", sysConfig.flotaInicio), where("fecha", "<=", sysConfig.flotaFin), orderBy("fecha", "desc"), limit(60))
            : query(collection(db, "registros_mantenimiento"), where("usuario", "==", currentUser.email), orderBy("fecha", "desc"), limit(30));
    ...
    const handleSubmit = async (e) => {
        ...
        if (Number(form.costo) < 0) return alert("Revisa el costo antes de enviar.");
        setIsUploading(true);
        try {
