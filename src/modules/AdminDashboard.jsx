   ...
   <div className="animate-in fade-in print-hide space-y-6">
                    ...
                    <div className="bg-[#151F32] p-6 rounded-[2rem] shadow-sm border border-slate-800">
                       <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                          <div>
                             <h4 className="font-bold text-slate-300 text-xs uppercase flex items-center gap-2"><TrendingUp size={16} className="text-green-500" /> Evolución Anual de Eficiencia (%)</h4>
                             <p className="text-[9px] text-slate-500 italic mt-1">💡 Resumen global anual con datos reales leídos al instante desde la Nube.</p>
                          </div>
                          {handleRebuildHistoricalSummaries && (
                             <button
                                type="button"
                                onClick={handleRebuildHistoricalSummaries}
                                disabled={isRebuildingSummaries}
                                title="Recalcular los 12 resúmenes mensuales con todos los registros reales de la base de datos"
                                className="bg-indigo-900/40 hover:bg-indigo-800/60 disabled:bg-slate-800 disabled:text-slate-600 text-indigo-300 border border-indigo-700/50 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase transition-all flex items-center gap-1.5 self-start sm:self-auto shadow-sm">
                                <RefreshCw size={12} className={isRebuildingSummaries ? "animate-spin text-indigo-400" : "text-indigo-400"} />
                                {isRebuildingSummaries ? "Recalculando..." : `Recalcular Histórico ${filterYear}`}
                             </button>
                          )}
                       </div>
