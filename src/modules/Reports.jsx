...
<div className="animate-in fade-in space-y-6 print-hide">
                   <div className="bg-[#151F32] p-6 rounded-[2rem] border border-slate-800 shadow-xl relative overflow-hidden">
                    ...
                    <div className="bg-[#151F32] p-6 rounded-[2rem] border border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <h3 className="text-xl font-black text-white flex items-center gap-2"><PieChartIcon className="text-indigo-500" /> Inteligencia de Negocios (YoY)</h3>
                            <p className="text-xs text-slate-400">Comparativa Anual Mensualizada ({biMetrics.yPrev} vs {biMetrics.yCurrent})</p>
                        </div>
                        {appMode === 'admin' && (
                            <button 
                                type="button" 
                                onClick={props.handleRebuildHistoricalSummaries} 
                                disabled={props.isRebuildingSummaries} 
                                title="Herramienta de mantenimiento: Ejecuta una lectura en lote para reconstruir los 12 documentos de resumen del año en Firestore." 
                                className="bg-slate-800/80 hover:bg-slate-700 disabled:bg-slate-900/50 disabled:text-slate-600 text-slate-300 border border-slate-700/80 px-3.5 py-2 rounded-xl text-[10px] font-bold uppercase flex items-center gap-2 shrink-0 transition-all shadow-sm">
                                {props.isRebuildingSummaries ? <Loader2 size={13} className="animate-spin text-indigo-400" /> : <RefreshCw size={13} className="text-slate-400" />} 
                                {props.isRebuildingSummaries ? 'Procesando año...' : `Recalcular Resúmenes ${filterYear}`}
                            </button>
                        )}
                    </div>
